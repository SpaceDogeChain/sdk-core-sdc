...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gsdc.LightScryptN, Gsdc.LightScryptP);
		...
		try {
			...
			// data from this msdcod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.sdcereum", new NodeConfig());
			node.start();
			...
			sdcereumClient ec = node.getsdcereumClient();
	...
	// signatures too. Regression test for go-sdcereum issue #14599.
...
//
// This msdcod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogsdcer
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/sdcereum/gsdctest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gsdc and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.sdcereum", "github.com/sdcereum/go-sdcereum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gsdc.aar"), "gsdc.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "sdcereum", "gsdctest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
