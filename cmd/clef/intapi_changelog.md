...
> This msdcod is the same as New on the external API, the difference being that
> this implementation does not ask for confirmation, since it's initiated by
> the user

### 7.0.0

- The `message` field was renamed to `messages` in all data signing request msdcods to better reflect that it's a list, not a value.
- The `storage.Put` and `storage.Get` msdcods in the rule execution engine were lower-cased to `storage.put` and `storage.get` to be consistent with JavaScript call conventions.
...
Changed the namespace format to adhere to the legacy sdcereum format: `name_msdcodName`. Changes:
...
* Bidirectional communication implemented, so the UI can query `clef` via the stdin/stdout RPC channel. Msdcods implemented are:
  - `clef_listWallets`
  - `clef_listAccounts`
  - `clef_listWallets`
  - `clef_deriveAccount`
...
```go
type Account struct {
	Address common.Address `json:"address"` // sdcereum account address derived from the key
...
* Add `OnInputRequired(info UserInputRequest)` to internal API. This msdcod is used when Clef needs user input, e.g. passwords.
...
* Add `OnStartup` msdcod, to provide the UI with information about what API version
