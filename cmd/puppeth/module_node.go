...
// nodeDockerfile is the Dockerfile required to run an spacedogechain node.
var nodeDockerfile = `
FROM spacedogechain/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.spacedogechain/keystore/ && cp /signer.json /root/.spacedogechain/keystore/' >> geth.sh && \{{end}}
...
// an spacedogechain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.spacedogechain{{if .Ethashdir}}
...
// deployNode deploys a new spacedogechain node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.spacedogechain"],
