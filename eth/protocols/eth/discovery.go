...
// enrEntry is the ENR entry which advertises `sdc` protocol on the discovery.
...
// StartENRUpdater starts the `sdc` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `sdc` ENR entry based on the current state of the chain.
