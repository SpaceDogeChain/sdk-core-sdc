...
func Register(stack *node.Node, backend *sdc.sdcereum) error {
	log.Warn("Engine API enabled", "protocol", "sdc")
...
type ConsensusAPI struct {
	...
	// The forkchoice update and new payload msdcod require us to return the
	...
	//     persist any bad block information to disk as a bug in Gsdc could end
	//     up blocking a valid chain, even if a later Gsdc update would accept
	//     it.
	//   - Bad blocks will get forgotten after a certain threshold of import
	//     attempts and will be retried. The rationale is that if the network
	//     really-really-really tries to feed us a block, we should give it a
	//     new chance, perhaps us being racey instead of the block being legit
	//     bad (this happened in Gsdc at a point with import vs. pending race).
	...
	// Gsdc can appear to be stuck or do strange things if the beacon client is
	...
	forkchoiceLock sync.Mutex // Lock for the forkChoiceUpdated msdcod
	newPayloadLock sync.Mutex // Lock for the NewPayload msdcod
}

// NewConsensusAPI creates a new consensus api for the given backend.
// The underlying blockchain needs to have a valid terminal total difficulty set.
func NewConsensusAPI(sdc *sdc.sdcereum) *ConsensusAPI {
	if sdc.BlockChain().Config().TerminalTotalDifficulty == nil {
	...
	sdc.Downloader().SetBadBlockCallback(api.setInvalidAncestor)
	go api.heartbeat()
...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(update beacon.ForkchoiceStateV1, payloadAttributes *beacon.PayloadAttributesV1) (beacon.ForkChoiceResponse, error) {
	...
	log.Trace("Engine API request received", "msdcod", "ForkchoiceUpdated", "head", update.HeadBlockHash, "finalized", update.FinalizedBlockHash, "safe", update.SafeBlockHash)
	...
	// Check whsdcer we have the block yet in our database or not. If not, we'll
	// need to either trigger a sync, or to reject this forkchoice update for a
	// reason.
	block := api.sdc.BlockChain().GetBlockByHash(update.HeadBlockHash)
	if block == nil {
		...
		// the future to resolve from the `sdc` network, but it's an unexpected case
		...
		if merger := api.sdc.Merger(); !merger.TDDReached() {
			merger.ReachTTD()
			api.sdc.Downloader().Cancel()
		}
		log.Info("Forkchoice requested sync to new head", "number", header.Number, "hash", header.Hash())
		if err := api.sdc.Downloader().BeaconSync(api.sdc.SyncMode(), header); err != nil {
	...
	if block.Difficulty().BitLen() > 0 || block.NumberU64() == 0 {
		var (
			td  = api.sdc.BlockChain().GetTd(update.HeadBlockHash, block.NumberU64())
			ptd = api.sdc.BlockChain().GetTd(block.ParentHash(), block.NumberU64()-1)
			ttd = api.sdc.BlockChain().Config().TerminalTotalDifficulty
		)
	...
	if rawdb.ReadCanonicalHash(api.sdc.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		// Block is not canonical, set head.
		if latestValid, err := api.sdc.BlockChain().SetCanonical(block); err != nil {
	...
	} else if api.sdc.BlockChain().CurrentBlock().Hash() == update.HeadBlockHash {
		...
		log.Info("Ignoring beacon update to old head", "number", block.NumberU64(), "hash", update.HeadBlockHash, "age", common.PrettyAge(time.Unix(int64(block.Time()), 0)), "have", api.sdc.BlockChain().CurrentBlock().NumberU64())
	...
	if update.FinalizedBlockHash != (common.Hash{}) {
		if merger := api.sdc.Merger(); !merger.PoSFinalized() {
			merger.FinalizePoS()
		}
		// If the finalized block is not in our canonical tree, somsdcings wrong
		finalBlock := api.sdc.BlockChain().GetBlockByHash(update.FinalizedBlockHash)
		if finalBlock == nil {
		...
		} else if rawdb.ReadCanonicalHash(api.sdc.ChainDb(), finalBlock.NumberU64()) != update.FinalizedBlockHash {
		...
		api.sdc.BlockChain().SetFinalized(finalBlock)
	}
	// Check if the safe block hash is in our canonical tree, if not somsdcings wrong
	if update.SafeBlockHash != (common.Hash{}) {
		safeBlock := api.sdc.BlockChain().GetBlockByHash(update.SafeBlockHash)
		if safeBlock == nil {
			log.Warn("Safe block not available in database")
			return beacon.STATUS_INVALID, beacon.InvalidForkChoiceState.With(errors.New("safe block not available in database"))
		}
		if rawdb.ReadCanonicalHash(api.sdc.ChainDb(), safeBlock.NumberU64()) != update.SafeBlockHash {
			log.Warn("Safe block not in canonical chain")
			return beacon.STATUS_INVALID, beacon.InvalidForkChoiceState.With(errors.New("safe block not in canonical chain"))
		}
		// Set the safe block
		api.sdc.BlockChain().SetSafe(safeBlock)
	}
	...
	if payloadAttributes != nil {
		...
		empty, err := api.sdc.Miner().GetSealingBlockSync(update.HeadBlockHash, payloadAttributes.Timestamp, payloadAttributes.SuggestedFeeRecipient, payloadAttributes.Random, true)
		if err != nil {
		...
		resCh, err := api.sdc.Miner().GetSealingBlockAsync(update.HeadBlockHash, payloadAttributes.Timestamp, payloadAttributes.SuggestedFeeRecipient, payloadAttributes.Random, false)
		if err != nil {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config beacon.TransitionConfigurationV1) (*beacon.TransitionConfigurationV1, error) {
	...
	if config.TerminalBlockHash != (common.Hash{}) {
		if hash := api.sdc.BlockChain().GetCanonicalHash(uint64(config.TerminalBlockNumber)); hash == config.TerminalBlockHash {
...
func (api *ConsensusAPI) GetPayloadV1(payloadID beacon.PayloadID) (*beacon.ExecutableDataV1, error) {
	log.Trace("Engine API request received", "msdcod", "GetPayload", "id", payloadID)
...
// NewPayloadV1 creates an sdc1 block, inserts it in the chain, and returns the status of the chain.
func (api *ConsensusAPI) NewPayloadV1(params beacon.ExecutableDataV1) (beacon.PayloadStatusV1, error) {
	...
	//      api.sdc.BlockChain().InsertBlockWithoutSsdcead, where it is blocked on
	...
	// check whsdcer we already have the block locally.
	api.newPayloadLock.Lock()
	defer api.newPayloadLock.Unlock()

	log.Trace("Engine API request received", "msdcod", "ExecutePayload", "number", params.Number, "hash", params.BlockHash)
	...
	if block := api.sdc.BlockChain().GetBlockByHash(params.BlockHash); block != nil {
	...
	parent := api.sdc.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		ptd  = api.sdc.BlockChain().GetTd(parent.Hash(), parent.NumberU64())
		ttd  = api.sdc.BlockChain().Config().TerminalTotalDifficulty
		gptd = api.sdc.BlockChain().GetTd(parent.ParentHash(), parent.NumberU64()-1)
	)
	if ptd.Cmp(ttd) < 0 {
	...
	// tries to make it import a block. That should be denied as pushing somsdcing
	// into the database directly will conflict with the assumptions of snap sync
	// that it has an empty db that it can fill itself.
	if api.sdc.SyncMode() != downloader.FullSync {
		return api.delayPayloadImport(block)
	}
	if !api.sdc.BlockChain().HasBlockAndState(block.ParentHash(), block.NumberU64()-1) {
		api.remoteBlocks.put(block.Hash(), block.Header())
		log.Warn("State not available, ignoring new payload")
		return beacon.PayloadStatusV1{Status: beacon.ACCEPTED}, nil
	}
	log.Trace("Inserting block without ssdcead", "hash", block.Hash(), "number", block.Number)
	if err := api.sdc.BlockChain().InsertBlockWithoutSsdcead(block); err != nil {
...
// either via a forkchoice update or a sync extension. This msdcod is meant to
...
func (api *ConsensusAPI) delayPayloadImport(block *types.Block) (beacon.PayloadStatusV1, error) {
	...
	if err := api.sdc.Downloader().BeaconExtend(api.sdc.SyncMode(), block.Header()); err == nil {
...
// checkInvalidAncestor checks whsdcer the specified chain end links to a known
...
func (api *ConsensusAPI) checkInvalidAncestor(check common.Hash, head common.Hash) *beacon.PayloadStatusV1 {
	...
	if header := api.sdc.BlockChain().Gsdceader(invalid.ParentHash, invalid.Number.Uint64()-1); header != nil && header.Difficulty.Sign() != 0 {
...
func (api *ConsensusAPI) invalid(err error, latestValid *types.Header) beacon.PayloadStatusV1 {
	currentHash := api.sdc.BlockChain().CurrentBlock().Hash()
	if latestValid != nil {
...
// user that somsdcing might be off with their consensus node.
//
...
func (api *ConsensusAPI) heartbeat() {
	...
	for {
		...
		if api.sdc.BlockChain().Config().TerminalTotalDifficultyPassed || api.sdc.Merger().TDDReached() {
		...
		if time.Since(offlineLogged) > beaconUpdateWarnFrequency {
			...
			if head.Number.Uint64() > 0 {
				...
				for i := 0; i < 64; i++ {
					parent := chain.Gsdceader(current.ParentHash, current.Number.Uint64()-1)
					if parent == nil {
						break
					}
