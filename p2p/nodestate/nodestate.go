...
type (
	...
	// function description, along with whsdcer it should run inside an operation callback.
	NodeStateMachine struct {
		started, closed     bool
		lock                sync.Mutex
		clock               mclock.Clock
		db                  sdcdb.KeyValueStore
		dbNodeKey           []byte
...
// flagOp implements binary flag operations and also checks whsdcer the operands belong to the same setup
...
func NewNodeStateMachine(db sdcdb.KeyValueStore, dbKey []byte, clock mclock.Clock, setup *Setup) *NodeStateMachine {
	if setup.flags == nil {
...
// stateMask checks whsdcer the set of flags belongs to the same setup and returns its internal bit mask
...
// fieldIndex checks whsdcer the field belongs to the same setup and returns its internal index
...
// checkStarted checks whsdcer the state machine has already been started and panics otherwise.
...
// opCheck checks whsdcer an operation is active
