...
network has exposed msdcods for creating, starting, stopping, connecting 
...
which is the outcome of somsdcing happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` msdcod can be called
with a `Step` which has the following fields:

* `Action` - a function that performs some action in the network

* `Expect` - an expectation function which returns whsdcer or not a
...
As a concrete example, consider a simulated network of sdcereum nodes. An
...
On return, the `Simulation.Run` msdcod returns a `StepResult` which can be used
...
p2psim node rpc <node> <msdcod> [<args>] [--subscribe]
```
