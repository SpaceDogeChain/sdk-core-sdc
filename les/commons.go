...
// NodeInfo represents a short summary of the sdcereum sub-protocol metadata
...
func (c *lesCommons) setupOracle(node *node.Node, genesis common.Hash, sdcconfig *sdcconfig.Config) *checkpointoracle.CheckpointOracle {
	config := sdcconfig.CheckpointOracle
	if config == nil {
	...
	client := sdcclient.NewClient(rpcClient)
	oracle.Start(client)
