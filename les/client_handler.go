...
func newClientHandler(ulcServers []string, ulcFraction int, checkpoint *params.TrustedCheckpoint, backend *Lightsdcereum) *clientHandler {
...
func (h *clientHandler) handle(p *serverPeer, noInitAnnounce bool) error {
	...
	if err := h.backend.peers.register(p); err != nil {
		p.Log().Error("Light sdcereum peer registration failed", "err", err)
		return err
	}
...
func (d *downloaderPeerNotify) registerPeer(p *serverPeer) {
	...
	h.downloader.RegisterLightPeer(p.id, sdc.sdc66, pc)
}
