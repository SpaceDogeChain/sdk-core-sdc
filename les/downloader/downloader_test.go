...
type downloadTester struct {
	...
	stateDb sdcdb.Database // Database used by the tester for syncing from peers
	peerDb  sdcdb.Database // Database of the peers containing all data
...
func (dl *downloadTester) HasHeader(hash common.Hash, number uint64) bool {
	return dl.GsdceaderByHash(hash) != nil
}
...
// Gsdceader retrieves a header from the testers canonical chain.
func (dl *downloadTester) GsdceaderByHash(hash common.Hash) *types.Header {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
	return dl.gsdceaderByHash(hash)
}

// gsdceaderByHash returns the header if found either within ancients or own blocks)
// This msdcod assumes that the caller holds at least the read-lock (dl.lock)
func (dl *downloadTester) gsdceaderByHash(hash common.Hash) *types.Header {
...
func (dl *downloadTester) InsertHeaderChain(headers []*types.Header, checkFreq int) (i int, err error) {
	...
	if dl.gsdceaderByHash(headers[0].ParentHash) == nil {
	...
	for i, header := range headers {
		hash := hashes[i]
		if dl.gsdceaderByHash(hash) != nil {
			continue
		}
		if dl.gsdceaderByHash(header.ParentHash) == nil {
...
func (dl *downloadTester) InsertChain(blocks types.Blocks) (i int, err error) {
	...
	for i, block := range blocks {
		...
		if hdr := dl.gsdceaderByHash(block.Hash()); hdr == nil {
...
func (dl *downloadTester) Ssdcead(head uint64) error {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
// RequestNodeData constructs a getNodeData msdcod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
func TestCanonicalSynchronisation66Fast(t *testing.T)  { testCanonSync(t, sdc.sdc66, FastSync) }
...
func TestThrottling66Fast(t *testing.T) { testThrottling(t, sdc.sdc66, FastSync) }
...
func TestForkedSync66Fast(t *testing.T)  { testForkedSync(t, sdc.sdc66, FastSync) }
...
func TestHeavyForkedSync66Fast(t *testing.T)  { testHeavyForkedSync(t, sdc.sdc66, FastSync) }
...
func TestBoundedForkedSync66Fast(t *testing.T)  { testBoundedForkedSync(t, sdc.sdc66, FastSync) }
...
func TestBoundedHeavyForkedSync66Fast(t *testing.T) {
	testBoundedHeavyForkedSync(t, sdc.sdc66, FastSync)
}
...
func TestCancel66Fast(t *testing.T)  { testCancel(t, sdc.sdc66, FastSync) }
...
func TestMultiSynchronisation66Fast(t *testing.T)  { testMultiSynchronisation(t, sdc.sdc66, FastSync) }
...
func TestMultiProtoSynchronisation66Fast(t *testing.T)  { testMultiProtoSync(t, sdc.sdc66, FastSync) }
...
func testMultiProtoSync(t *testing.T, protocol uint, mode SyncMode) {
	...
	tester.newPeer("peer 66", sdc.sdc66, chain)
	//tester.newPeer("peer 65", sdc.sdc67, chain)
...
func TestEmptyShortCircuit66Fast(t *testing.T)  { testEmptyShortCircuit(t, sdc.sdc66, FastSync) }
...
func TestMissingHeaderAttack66Fast(t *testing.T)  { testMissingHeaderAttack(t, sdc.sdc66, FastSync) }
...
func TestShiftedHeaderAttack66Fast(t *testing.T)  { testShiftedHeaderAttack(t, sdc.sdc66, FastSync) }
...
func TestInvalidHeaderRollback66Fast(t *testing.T) { testInvalidHeaderRollback(t, sdc.sdc66, FastSync) }
...
func TestHighTDStarvationAttack66Fast(t *testing.T) {
	testHighTDStarvationAttack(t, sdc.sdc66, FastSync)
}
...
func TestSyncProgress66Fast(t *testing.T)  { testSyncProgress(t, sdc.sdc66, FastSync) }
...
func TestForkedSyncProgress66Fast(t *testing.T)  { testForkedSyncProgress(t, sdc.sdc66, FastSync) }
...
func TestFailedSyncProgress66Fast(t *testing.T)  { testFailedSyncProgress(t, sdc.sdc66, FastSync) }
...
func TestFakedSyncProgress66Fast(t *testing.T)  { testFakedSyncProgress(t, sdc.sdc66, FastSync) }
...
func TestDeliverHeadersHang66Full(t *testing.T)  { testDeliverHeadersHang(t, sdc.sdc66, FullSync) }
func TestDeliverHeadersHang66Fast(t *testing.T)  { testDeliverHeadersHang(t, sdc.sdc66, FastSync) }
func TestDeliverHeadersHang66Light(t *testing.T) { testDeliverHeadersHang(t, sdc.sdc66, LightSync) }
...
func TestCheckpointEnforcement66Fast(t *testing.T) { testCheckpointEnforcement(t, sdc.sdc66, FastSync) }
