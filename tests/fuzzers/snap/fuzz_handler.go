...
func getChain() *core.BlockChain {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 2, func(i int, gen *core.BlockGen) {})
	...
	bc, _ := core.NewBlockChain(rawdb.NewMemoryDatabase(), cacheConf, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
