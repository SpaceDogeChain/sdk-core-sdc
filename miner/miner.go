...
// Backend wraps all msdcods required for mining. Only full node is capable
...
type Config struct {
	sdcerbase  common.Address `toml:",omitempty"` // Public address for block mining rewards (default = first account)
	Notify     []string       `toml:",omitempty"` // HTTP URL list to be notified of new work packages (only useful in sdcash).
	...
	Noverify   bool           // Disable remote mining solution verification(only useful in sdcash).
...
func New(sdc Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(header *types.Header) bool) *Miner {
	miner := &Miner{
		sdc:     sdc,
		mux:     mux,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, sdc, mux, isLocalBlock, true),
	}
	miner.wg.Add(1)
	go miner.update()
	return miner
	...
	for {
		...
		case ev := <-dlEventCh:
			...
			case downloader.FailedEvent:
				canStart = true
				if shouldStart {
					miner.Setsdcerbase(miner.coinbase)
					miner.worker.start()
				}
...
// change between multiple msdcod calls
...
func (miner *Miner) Setsdcerbase(addr common.Address) {
	miner.coinbase = addr
	miner.worker.setsdcerbase(addr)
}
...
// given parameters. Regardless of whsdcer the generation is successful or not,
