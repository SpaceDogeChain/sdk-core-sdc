...
const ourPath = "github.com/sdcereum/go-sdcereum" // Path to our module
...
// conventions in the sdcereum p2p network.
...
//
// If the package that is currently executing is a prefixed by our go-sdcereum
...
// version and whsdcer it was replaced by another module.
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gsdc".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gsdc %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gsdc %s", params.VersionWithMeta)
	}
	...
	if mod.Replace != nil {
		// If our package was replaced by somsdcing else, also note that.
