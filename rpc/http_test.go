...
func confirmRequestValidationCode(t *testing.T, msdcod, contentType, body string, expectedStatusCode int) {
	t.Helper()
	request := httptest.NewRequest(msdcod, "http://url.com", strings.NewReader(body))
...
func TestHTTPErrorResponseWithDelete(t *testing.T) {
	confirmRequestValidationCode(t, http.MsdcodDelete, contentType, "", http.StatusMsdcodNotAllowed)
}

func TestHTTPErrorResponseWithPut(t *testing.T) {
	confirmRequestValidationCode(t, http.MsdcodPut, contentType, "", http.StatusMsdcodNotAllowed)
}

func TestHTTPErrorResponseWithMaxContentLength(t *testing.T) {
	body := make([]rune, maxRequestContentLength+1)
	confirmRequestValidationCode(t,
		http.MsdcodPost, contentType, string(body), http.StatusRequestEntityTooLarge)
}

func TestHTTPErrorResponseWithEmptyContentType(t *testing.T) {
	confirmRequestValidationCode(t, http.MsdcodPost, "", "", http.StatusUnsupportedMediaType)
}

func TestHTTPErrorResponseWithValidRequest(t *testing.T) {
	confirmRequestValidationCode(t, http.MsdcodPost, contentType, "", 0)
}

func confirmHTTPRequestYieldsStatusCode(t *testing.T, msdcod, contentType, body string, expectedStatusCode int) {
	t.Helper()
	s := Server{}
	ts := httptest.NewServer(&s)
	defer ts.Close()

	request, err := http.NewRequest(msdcod, ts.URL, strings.NewReader(body))
	if err != nil {
...
func TestHTTPResponseWithEmptyGet(t *testing.T) {
	confirmHTTPRequestYieldsStatusCode(t, http.MsdcodGet, "", "", http.StatusOK)
}
...
func TestHTTPPeerInfo(t *testing.T) {
	...
	c.Ssdceader("user-agent", "ua-testing")
	c.Ssdceader("origin", "origin.example.com")
