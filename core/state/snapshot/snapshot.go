...
// additional msdcods compared to the public API.
type snapshot interface {
	Snapshot
	...
	// reached.
	//
	// Note, the msdcod is an internal helper to avoid type switching between the
	...
	//
	// Note, the maps are retained by the msdcod to avoid copying everything.
	...
	// Stale return whsdcer this layer has become stale (was flattened across) or
	// if it's still live.
	Stale() bool
...
// Tree is an sdcereum state snapshot tree. It consists of one persistent base
...
type Tree struct {
	...
	diskdb sdcdb.KeyValueStore      // Persistent database to store the snapshot
...
//     a background thread.
func New(config Config, diskdb sdcdb.KeyValueStore, triedb *trie.Database, root common.Hash) (*Tree, error) {
...
// waitBuild blocks until the snapshot finishes rebuilding. This msdcod is meant
...
//
// The msdcod returns the new disk layer if diffs were persisted into it.
//
// Note, the final diff layer count in general will be one more than the amount
// requested. This happens because the bottom-most diff layer is the accumulator
// which may or may not overflow and cascade to disk. Since this last layer's
...
// it. The msdcod will panic if called onto a non-bottom-most diff layer.
//
...
// flattening everything down (bad for reorgs).
//
// The msdcod returns the root hash of the base layer that needs to be persisted
...
func (t *Tree) Verify(root common.Hash) error {
	...
	got, err := generateTrieRoot(nil, acctIt, common.Hash{}, stackTrieGenerate, func(db sdcdb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, error) {
...
// generating is an internal helper function which reports whsdcer the snapshot
