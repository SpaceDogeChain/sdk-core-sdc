...
func newCanonical(engine consensus.Engine, n int, full bool) (sdcdb.Database, *Genesis, *BlockChain, error) {
	var (
		genesis = &Genesis{
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	genDb, _, blockchain2, err := newCanonical(sdcash.NewFaker(), i, full)
	if err != nil {
	...
	if full {
		...
		hash1 = blockchain.GsdceaderByNumber(uint64(i)).Hash()
		hash2 = blockchain2.GsdceaderByNumber(uint64(i)).Hash()
	}
	if hash1 != hash2 {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.chainConfig, blockchain2.CurrentBlock(), n, sdcash.NewFaker(), genDb, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.chainConfig, blockchain2.CurrentHeader(), n, sdcash.NewFaker(), genDb, forkSeed)
...
func TestLastBlock(t *testing.T) {
	genDb, _, blockchain, err := newCanonical(sdcash.NewFaker(), 0, true)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 1, sdcash.NewFullFaker(), genDb, 0)
...
func testExtendCanonical(t *testing.T, full bool) {
	...
	_, _, processor, err := newCanonical(sdcash.NewFaker(), length, full)
	if err != nil {
...
func testBrokenChain(t *testing.T, full bool) {
	...
	genDb, _, blockchain, err := newCanonical(sdcash.NewFaker(), 10, full)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 5, sdcash.NewFaker(), genDb, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 5, sdcash.NewFaker(), genDb, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	genDb, _, blockchain, err := newCanonical(sdcash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	...
	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), sdcash.NewFaker(), genDb, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), sdcash.NewFaker(), genDb, len(second), func(i int, b *BlockGen) {
	...
	if full {
		...
		for header := blockchain.GsdceaderByNumber(blockchain.CurrentHeader().Number.Uint64() - 1); header.Number.Uint64() != 0; prev, header = header, blockchain.GsdceaderByNumber(header.Number.Uint64()-1) {
...
func testBadHashes(t *testing.T, full bool) {
	...
	if full {
		blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 3, sdcash.NewFaker(), genDb, 10)
		...
		headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 3, sdcash.NewFaker(), genDb, 10)
...
func testReorgBadHashes(t *testing.T, full bool) {
	// Create a pristine chain and database
	genDb, gspec, blockchain, err := newCanonical(sdcash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	// Create a chain, import and ban afterwards
	headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 4, sdcash.NewFaker(), genDb, 10)
	blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 4, sdcash.NewFaker(), genDb, 10)
	...
	ncm, err := NewBlockChain(blockchain.db, nil, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool) {
	doTest := func(i int) {
		...
		if full {
			blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), i, sdcash.NewFaker(), genDb, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.engine = sdcash.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), i, sdcash.NewFaker(), genDb, 0)
		...
		for j := 0; j < i-failAt; j++ {
			if full {
				...
				if header := blockchain.GsdceaderByNumber(failNum + uint64(j)); header != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	_, blocks, receipts := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 1024, func(i int, block *BlockGen) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
	...
	for i := 0; i < len(blocks); i++ {
		...
		if fheader, aheader := fast.GsdceaderByHash(hash), archive.GsdceaderByHash(hash); fheader.Hash() != aheader.Hash() {
			t.Errorf("block #%d [%x]: header mismatch: fastdb %v, archivedb %v", num, hash, fheader, aheader)
		}
		if anheader, arheader := ancient.GsdceaderByHash(hash), archive.GsdceaderByHash(hash); anheader.Hash() != arheader.Hash() {
...
// Tests that various import msdcods move the chain head pointers to the correct
// positions.
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	height := uint64(1024)
	_, blocks, receipts := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), int(height), nil)

	// makeDb creates a db instance for testing.
	makeDb := func() sdcdb.Database {
	...
	// Create a small assertion msdcod to check the three heads
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	lightDb := makeDb()
	defer lightDb.Close()
	light, _ := NewBlockChain(lightDb, nil, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	blockchain, _ := NewBlockChain(db, nil, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
	...
	_, chain, _ = GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func TestLogReorgs(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
	defer blockchain.Stop()

	rmLogsCh := make(chan RemovedLogsEvent)
	blockchain.SubscribeRemovedLogsEvent(rmLogsCh)
	_, chain, _ := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
...
func TestSideLogRebirth(t *testing.T) {
	...
	genDb, sideChain, _ := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	newBlocks, _ := GenerateChain(gspec.Config, sideChain[len(sideChain)-1], sdcash.NewFaker(), genDb, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
...
func TestReorgSideEvent(t *testing.T) {
	...
	_, replacementBlocks, _ := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 4, func(i int, gen *BlockGen) {
...
func TestCanonicalBlockRetrieval(t *testing.T) {
	_, gspec, blockchain, err := newCanonical(sdcash.NewFaker(), 0, true)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	_, chain, _ := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 10, func(i int, gen *BlockGen) {})
...
func TestEIP155Transition(t *testing.T) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 4, func(i int, block *BlockGen) {
		var (
	...
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], sdcash.NewFaker(), genDb, 4, func(i int, block *BlockGen) {
		var (
			tx      *types.Transaction
			err     error
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
func TestEIP161AccountRemoval(t *testing.T) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.LatestSigner(gspec.Config)
...
//
// https://github.com/sdcereum/go-sdcereum/pull/15941
...
func TestInsertReceiptChainRollback(t *testing.T) {
	...
	ancientChain, _ := NewBlockChain(ancientDb, nil, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, nil)
...
//
// Details at:
//   - https://github.com/sdcereum/go-sdcereum/issues/18977
//   - https://github.com/sdcereum/go-sdcereum/pull/18988
func TestLowDiffLongChain(t *testing.T) {
	...
	for number := head.NumberU64(); number > 0; number-- {
		if hash := chain.GsdceaderByNumber(number).Hash(); hash != header.Hash() {
			t.Fatalf("header %d: canonical hash mismatch: have %x, want %x", number, hash, header.Hash())
		}
		header = chain.Gsdceader(header.ParentHash, number-1)
	}
}
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	var (
		...
		engine = beacon.New(sdcash.NewFaker())
		key, _ = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
...
func TestPrunedImportSide(t *testing.T) {
	...
	//glogger.Verbosity(3)
	//log.Root().Ssdcandler(log.Handler(glogger))
...
func TestReorgToShorterRemovesCanonMapping(t *testing.T) {
	...
	if headerByNum := chain.GsdceaderByNumber(canonNum); headerByNum != nil {
		t.Errorf("expected header to be gone: %v", headerByNum.Number.Uint64())
	}
	if blockByHash := chain.GetBlockByHash(canonHash); blockByHash == nil {
		t.Errorf("expected block to be present: %x", blockByHash.Hash())
	}
	if headerByHash := chain.GsdceaderByHash(canonHash); headerByHash == nil {
...
func TestTransactionIndices(t *testing.T) {
	...
	_, blocks, receipts := GenerateChainWithGenesis(gspec, sdcash.NewFaker(), 128, func(i int, block *BlockGen) {
		tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, block.header.BaseFee, nil), signer, key)
		if err != nil {
			panic(err)
		}
	...
	for _, l := range limit {
		...
		l := l
		chain, err := NewBlockChain(ancientDb, nil, gspec, nil, sdcash.NewFaker(), vm.Config{}, nil, &l)
		if err != nil {
			t.Fatalf("failed to create tester chain: %v", err)
		}
...
//     with 0.001 sdcer (thus created but no code)
...
// to the destructset in case somsdcing is created "onto" an existing item.
...
func TestEIP1559Transition(t *testing.T) {
	var (
		...
		addr1   = crypto.PubkeyToAddress(key1.PublicKey)
		addr2   = crypto.PubkeyToAddress(key2.PublicKey)
		funds   = new(big.Int).Mul(common.Big1, big.NewInt(params.sdcer))
		gspec   = &Genesis{
			Config: params.AllsdcashProtocolChanges,
			Alloc: GenesisAlloc{
				addr1: {Balance: funds},
	...
	expected := new(big.Int).Add(
		...
		sdcash.ConstantinopleBlockReward,
	)
...
func TestSetCanonical(t *testing.T) {
	...
	for _, block := range side {
		err := chain.InsertBlockWithoutSsdcead(block)
		if err != nil {
...
func TestTxIndexer(t *testing.T) {
	...
	verifyIndexes := func(db sdcdb.Database, number uint64, exist bool) {
		if number == 0 {
			return
		}
	...
	verifyRange := func(db sdcdb.Database, from, to uint64, exist bool) {
		for number := from; number <= to; number += 1 {
			verifyIndexes(db, number, exist)
		}
	}
	verify := func(db sdcdb.Database, expTail uint64) {
