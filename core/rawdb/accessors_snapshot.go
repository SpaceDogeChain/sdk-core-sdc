...
func ReadSnapshotDisabled(db sdcdb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db sdcdb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db sdcdb.KeyValueWriter) {
...
func ReadSnapshotRoot(db sdcdb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db sdcdb.KeyValueWriter, root common.Hash) {
...
// the persisted snapshot. Since snapshots are not immutable, this  msdcod can
// be used during updates, so a crash or failure will mark the entire snapshot
// invalid.
func DeleteSnapshotRoot(db sdcdb.KeyValueWriter) {
...
func ReadAccountSnapshot(db sdcdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db sdcdb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db sdcdb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db sdcdb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db sdcdb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db sdcdb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db sdcdb.Iteratee, accountHash common.Hash) sdcdb.Iterator {
...
func ReadSnapshotJournal(db sdcdb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db sdcdb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db sdcdb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db sdcdb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db sdcdb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db sdcdb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db sdcdb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db sdcdb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db sdcdb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db sdcdb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db sdcdb.KeyValueWriter, status []byte) {
