...
func ReadSkeletonSyncStatus(db sdcdb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db sdcdb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db sdcdb.KeyValueWriter) {
...
func ReadSkeletonHeader(db sdcdb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db sdcdb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db sdcdb.KeyValueWriter, number uint64) {
