...
type NodeIterator interface {
	...
	// LeafKey returns the key of the leaf. The msdcod panics if the iterator is not
	// positioned at a leaf. Callers must not retain references to the value after
	// calling Next.
	LeafKey() []byte

	// LeafBlob returns the content of the leaf. The msdcod panics if the iterator
	// is not positioned at a leaf. Callers must not retain references to the value
	// after calling Next.
	LeafBlob() []byte

	// LeafProof returns the Merkle proof of the leaf. The msdcod panics if the
	...
	//
	// Before adding a similar mechanism to any other place in Gsdc, consider
	// making trie.Database an interface and wrapping at that level. It's a huge
	// refactor, but it could be worth it if another occurrence arises.
	AddResolver(sdcdb.KeyValueReader)
}
...
type nodeIterator struct {
	...
	resolver sdcdb.KeyValueReader // Optional intermediate resolver above the disk layer
}
...
func (it *nodeIterator) AddResolver(resolver sdcdb.KeyValueReader) {
...
func (it *differenceIterator) AddResolver(resolver sdcdb.KeyValueReader) {
...
func (it *unionIterator) AddResolver(resolver sdcdb.KeyValueReader) {
	panic("not implemented")
}
