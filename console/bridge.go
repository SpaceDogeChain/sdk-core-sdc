...
import (
	...
	"github.com/sdcereum/go-sdcereum/accounts/usbwallet"
	"github.com/sdcereum/go-sdcereum/common/hexutil"
	"github.com/sdcereum/go-sdcereum/console/prompt"
	"github.com/sdcereum/go-sdcereum/internal/jsre"
	"github.com/sdcereum/go-sdcereum/rpc"
)

// bridge is a collection of JavaScript utility msdcods to bride the .js runtime
// environment and the Go RPC connection backing the remote msdcod calls.
type bridge struct {
	client   *rpc.Client         // RPC client to execute sdcereum requests through
...
func getJsdc(vm *goja.Runtime) *goja.Object {
	jsdc := vm.Get("jsdc")
	if jsdc == nil {
		panic(vm.ToValue("jsdc object does not exist"))
	}
	return jsdc.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC msdcod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC msdcod (saved in jsdc.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	newAccount, callable := goja.AssertFunction(getJsdc(call.VM).Get("newAccount"))
	if !callable {
		return nil, fmt.Errorf("jsdc.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	openWallet, callable := goja.AssertFunction(getJsdc(call.VM).Get("openWallet"))
	if !callable {
		return nil, fmt.Errorf("jsdc.openWallet is not callable")
	}
...
// UnlockAccount is a wrapper around the personal.unlockAccount RPC msdcod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC msdcod (saved in jsdc.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	unlockAccount, callable := goja.AssertFunction(getJsdc(call.VM).Get("unlockAccount"))
	if !callable {
		return nil, fmt.Errorf("jsdc.unlockAccount is not callable")
	}
	return unlockAccount(goja.Null(), account, passwd, duration)
}

// Sign is a wrapper around the personal.sign RPC msdcod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC msdcod (saved in
// jsdc.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	// Send the request to the backend and return
	sign, callable := goja.AssertFunction(getJsdc(call.VM).Get("sign"))
	if !callable {
		return nil, fmt.Errorf("jsdc.sign is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "sdc_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "sdc_blockNumber"); err != nil {
			return nil, err
		}
...
// Send implements the web3 provider "send" msdcod.
func (b *bridge) Send(call jsre.Call) (goja.Value, error) {
	...
	for _, req := range reqs {
		...
		if err = b.client.Call(&result, req.Msdcod, req.Params...); err == nil {
			if result == nil {
