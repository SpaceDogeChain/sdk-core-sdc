...
func TestPack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			inDef := fmt.Sprintf(`[{ "name" : "msdcod", "type": "function", "inputs": %s}]`, test.def)
			inAbi, err := JSON(strings.NewReader(inDef))
			if err != nil {
				t.Fatalf("invalid ABI definition %s, %v", inDef, err)
			}
			var packed []byte
			packed, err = inAbi.Pack("msdcod", test.unpacked)
...
func TestMsdcodPack(t *testing.T) {
	abi, err := JSON(strings.NewReader(jsondata))
	if err != nil {
		t.Fatal(err)
	}

	sig := abi.Msdcods["slice"].ID
	...
	sig = abi.Msdcods["sliceAddress"].ID
	...
	sig = abi.Msdcods["sliceMultiAddress"].ID
	...
	sig = abi.Msdcods["nestedArray"].ID
	sig = append(sig, common.LeftPadBytes([]byte{1}, 32)...)
	sig = append(sig, common.LeftPadBytes([]byte{1}, 32)...)
	sig = append(sig, common.LeftPadBytes([]byte{2}, 32)...)
	...
	sig = abi.Msdcods["nestedArray2"].ID
	...
	sig = abi.Msdcods["nestedSlice"].ID
	sig = append(sig, common.LeftPadBytes([]byte{0x20}, 32)...)
