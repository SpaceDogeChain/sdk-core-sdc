...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-sdc/master/doc/sdcapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and sdcereum address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an sdcereum transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
	ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs an sdcereum message following the EIP 712 specification
...
type ledgerDriver struct {
	...
	browser bool          // Flag whsdcer the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "sdcereum app in browser mode", w.failure
	}
	if w.offline() {
		return "sdcereum app offline", w.failure
	}
	return fmt.Sprintf("sdcereum app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whsdcer the wallet and the sdcereum app is offline or not.
//
// The msdcod assumes that the state lock is held!
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// sdcereum app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the sdcereum app's version, will fail prior to v1.0.2
...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the sdcereum app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the sdcereum wallet app running
...
// ledgerDerive retrieves the currently active sdcereum address from a Ledger
...
//	sdcereum address length | 1 byte
//	sdcereum address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the sdcereum hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an sdcereum address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whsdcer legacy or EIP155 signing was requested
	var (
		txrlp []byte
		err   error
	)
